...
from tables import TABLES, get_table_key as get_table_key_from_schema, get_table_schema, is_table_exists as is_table_exists_in_schema
...
async def get_db_array(request: GetDbArrayRequest):
    ...
    try:
        ...
        sql = f'SELECT {fields} FROM {table_name(table)}{where_clause}{order_clause}'
...
async def rest_access_get(table_name: str, request: Request):
    ...
    try:
        ...
        sql = f'SELECT * FROM {table_name(table_name)} WHERE {condition}{order_clause}'
...
async def rest_access_post(table_name: str, request: Request):
    ...
    try:
        ...
        sql = f'INSERT INTO {table_name(table_name)} ({", ".join(keys)}) VALUES ({", ".join(values)}) RETURNING *'
...
async def rest_access_put(table_name: str, request: Request):
    ...
    try:
        ...
        sql = f'UPDATE {table_name(table_name)} SET {set_clause} WHERE {where_clause} RETURNING *'
...
async def rest_access_delete(table_name: str, request: Request):
    ...
    try:
        ...
        # DELETE 쿼리 생성
        where_clause = get_conditions(table_name, keys, len(keys))
        sql = f'DELETE FROM {table_name(table_name)} WHERE {where_clause} RETURNING *'
