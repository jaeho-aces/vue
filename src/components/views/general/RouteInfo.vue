...
const columns: TableColumn[] = [
  ...
  { id: 'registered', header: '등록 일자', size: columnWidths[6], cellComponent: DateOnlyCell }
]
...
// GRP_GBN='4' (노선) 데이터를 Route 형식으로 변환. 수정 키는 (그룹구분, 코드) 2개만 사용.
...
const handleDataUpdate = async (data: Record<string, any>, isNew: boolean) => {
  try {
    ...
    if (isNew) {
      ...
      // 수정: 키는 (그룹구분, 코드) 2개. id 형식 "grpGbn_code"
...
// id: 수정 시 복합 키 파싱용 (모달 제출 데이터에 id 포함)
const formFields: FormField[] = [
  { id: 'id', type: 'hidden' },
  { id: 'code', label: '코드', type: 'text', required: true, placeholder: '예: RT01', readonlyInEdit: true, maxLength: 5 },
  { id: 'fullName', label: '정식 노선 이름', type: 'text', required: true, placeholder: '예: 경부고속도로', maxLength: 500 },
  { id: 'shortName', label: '간략 노선 이름', type: 'text', required: true, placeholder: '예: 경부선', maxLength: 100 },
