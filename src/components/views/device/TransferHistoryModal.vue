  ...
  >
    ...
    >
      <div class="px-4 py-3 border-b border-slate-200 flex items-center justify-between">
        <h3 class="text-lg font-semibold text-slate-800">전송 이력</h3>
        <button
          type="button"
...
const lastFetchedMinutes = ref<number[]>([]) // 1440: 분 단위 전송 성공 0/1 (API에서 오면 저장)
const isLoading = ref(false)
const selectedHour = ref<number | null>(null)
/** 선택한 시간대의 0~59분 전송 여부(1/0). 선택 시 한 번만 계산해 고정 */
...
function onHourBarHover(_event: unknown, elements: unknown[]) {
  const el = document.querySelector('.transfer-history-chart-wrap') as HTMLElement | null
...
/** 선택한 시간대의 0~59분 전송 여부 (1=성공, 0=미전송). selectedHour 변경 시 한 번 계산해 저장됨 */
...
function updateMinuteDetailsForSelectedHour() {
  ...
  const arr: number[] = Array.from({ length: 60 }, (_, i) => (i < count ? 1 : 0))
  for (let i = 59; i > 0; i--) {
  ...
  datasets: [
    {
      ...
      backgroundColor: minuteDetails.value.map((v) => (v ? '#34d399' : '#f1f5f9')),
      borderColor: 'transparent',
      borderWidth: 0,
      borderRadius: 2
    }
  ...
  scales: {
    ...
    y: {
      ...
      ticks: {
        color: '#64748b',
        stepSize: 1,
        callback: (value: unknown) => (value === 1 ? '성공' : value === 0 ? '미전송' : '')
      }
    }
  }
}
...
async function loadHistory() {
  ...
  try {
    const data = await api.getTransferHistory(props.channelId, selectedDate.value)
