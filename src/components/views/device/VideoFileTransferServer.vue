...
// 기본 표시 컬럼
const defaultVisibleColumns = ['fts_name', 'fts_type', 'ip_addr', 'fts_target_ip', 'reg_date']
...
const formFields = computed<FormField[]>(() => [
  { id: 'fts_id', label: '영상 파일 전송 서버 ID', type: 'text', required: true, placeholder: 'FTSXX', maxLength: 5, readonlyInEdit: true },
...
const handleDataUpdate = async (data: Record<string, any>, isNew: boolean) => {
  if (isSubmitting.value || transferServerStore.isLoading) return
  try {
    isSubmitting.value = true
    if (isNew) {
      const fts_id = String(data.fts_id ?? '').trim().slice(0, 5)
    ...
    await transferServerStore.fetchTransferServers(true)
