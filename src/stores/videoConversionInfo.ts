  ...
  actions: {
    ...
    transformFromAPI(data: any): VideoConversion {
      return {
        ...
        ch_wmv_vkpbs: data.ch_wmv_vkpbs || '',
        sms_session: data.sms_session || '',
        sms_host_ip: data.sms_host_ip || '',
        sms_date: data.sms_date || null,
    ...
    async mapCodeNames() {
      ...
      const cctvIds = [...new Set(
        this.items
          .map((item: VideoConversion) => item.cctv_id)
      ...
      try {
        ...
        const updatedItems = this.items.map((item: VideoConversion) => {
          const cctvId = (item.cctv_id || '').toLowerCase()
